...
public class Purifyjava {
	public static void main(String[] args) {
		MongoClient mongoClient = new MongoClient();
		MongoDatabase db = mongoClient.getDatabase(MongoDBUtil.DB_NAME);
		String fileName = "/Users/junyihuang/Downloads/tomcat_log.txt";
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				Pattern pattern = Pattern.compile("\\[(.+?):(.+)"); // 匹配(日期) : (时间), .:任何字符
				// : 见到 ： 就停，尽量短进行匹配，（）：匹配任意长的字符串
				Matcher matcher = pattern.matcher(timestamp); // 进行时间匹配
			    matcher.find();
