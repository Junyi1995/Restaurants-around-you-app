...
public class FindPeak {
	...
	public static void main(String [] args) {
		...
		// MongoDB function construction, like JavaScript : 
		// Construct mapper function
		// function() {
		// 		if(this.url.startswith("/Titan")){ // if it begins with Titan
		//			emit(this.time.substring(0, 5), 1); // send out to reducer, only hour and minute
		//		}
		// }
		
		// use StringBuilder to build javaSript function
		StringBuilder sb = new StringBuilder();
		sb.append("function() {");
		sb.append("if (this.url.startsWith(\"");
		sb.append(URL_PREFIX);
		sb.append("\")) { emit(this.time");
	...
	// output : "00:00"
		private static String findBucket(String currentTime) {
			...
			while (left < right - 1) {
				...
				if (buckets.get(mid).isAfter(curr)) { // binary search
					right = mid - 1;
				} else {
					left = mid;
				}
